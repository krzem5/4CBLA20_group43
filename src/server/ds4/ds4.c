...
static const uint32_t _ds4_packet_crc_table[256]={
...
void ds4_recv(ds4_device_t* device){
	...
	device->has_wired_connection=(buffer[0]==0x11);
	const uint8_t* ptr=buffer+(device->has_wired_connection<<1);
	device->buttons=0;
...
void ds4_send(const ds4_device_t* device){
	...
	buffer[4]=0xf3;
	buffer[5]=0x04;
	buffer[7]=device->rumble_small;
	buffer[8]=device->rumble_big;
	buffer[9]=device->led_red;
	buffer[10]=device->led_green;
	buffer[11]=device->led_blue;
	uint32_t crc=0xffffffff;
	for (uint32_t i=0;i<75;i++){
		crc=_ds4_packet_crc_table[(crc&0xff)^buffer[i]]^(crc>>8);
	}
	crc=~crc;
	buffer[75]=crc;
	buffer[76]=crc>>8;
